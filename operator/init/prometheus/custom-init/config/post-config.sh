...
cp /amq/prometheus-plugin/artemis-prometheus-metrics-plugin-1.1.0.CR1/artemis-prometheus-metrics-plugin-servlet/target/metrics.jar ${CONFIG_INSTANCE_DIR}/web
ls ${CONFIG_INSTANCE_DIR}/web

sed -i 's/<\/binding>/           <app url="metrics" war="metrics.war"/>\n       /g' ${CONFIG_INSTANCE_DIR}/etc/bootstrap.xml
